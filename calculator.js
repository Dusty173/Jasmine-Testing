...
function setupIntialValues() {
  let values = {amount: 30000, years: 5, rate: 4.5 };
...
function update() {
  const initInputvalues = getCurrentUIValues();
  updateMonthly(calculateMonthlyPayment(initInputvalues));
}
...
function calculateMonthlyPayment(values) {
  ...
  const yearBreak = Math.floor(values.years * 12);
  return (
    (monthlyRate * values.amount) / 
    (1 - Math.pow((1 + monthlyRate) -yearBreak))
    ).toFixed(2);
}
